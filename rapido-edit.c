...
static void usage(const char *cmd) {
    printf("Usage: %s [options] host port\n"
           ...
           "  -a address           additional ip addresses the server has, cannot be repeated (yet ?), ignored if -c is not used\n"
...
int main(int argc, char **argv) {
    ...
    const char *host, *port, *extra_host = NULL;
    int is_server = 0, ch, n_extra_host = 0;
